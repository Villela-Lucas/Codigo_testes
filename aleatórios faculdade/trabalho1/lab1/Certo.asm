...
size_exceeded:				#MENSAGEM DE ERRO PARA QUANDO UM CARACTERE UM IVALIDO Eh DIGITADO NO MENU
	...
	li $a1, 2			#FAZ APARECER O SIMBOLO DE ERRO
	syscall				#FAZ A CHAMADA AO SISTEMA PARA APARECER A MENSAGEM AO USUARIO
	beq $s4, '2', cont		#SIGNIFICA QUE O ERRO OCORREU QUANDO O USUARIO ESTAVA EDITANDO UM REGISTRO (VAI PARA cont)
	beq $s4, '3', compara		#SIGNIFICA QUE O ERRO OCORREU QUANDO O USUARIO ESTAVA CONSULTANDO UM REGISTRO (VAI PARA compara)
	beq $s4, '5', cont		#SIGNIFICA QUE O ERRO OCORREU QUANDO O USUARIO ESTAVA APAGANDO UM REGISTRO (VAI PARA cont)
...
#################READ FILE##########################
	readfile:				#LE O ARQUIVO
		...
		move $a0, $s0			#COLOCA $s0 (file descriptor) em $a0
		syscall				#CHAMADA DE SISTEMA (FECHA O ARQUIVO)
		beq $s4, '2', apaga		#SE $s4 FOR '2' (O USUARIO ESTA EDITANDO UM REGISTRO), VAI PARA apaga
		...
		beq $s4, '5', apaga		#SE $s4 FOR '5' (O USUARIO ESTA APAGANDO UM REGISTRO), VAI PARA apaga
#####################NUMERO DE LINHAS####################
...
compara:					
	...
	bne $s4, '3', colhe_id			#SE $s4 FOR DIFERENTE DE '3', VAI PARA colhe_id
	muda:					
...
close_file:						#FECHA file (ESTAVA SENDO ESCRITO EM file)
	...
	beq $s4, '1', menu				#SE A ESCOLHA FOI CRIAR UM NOVO REGISTRO ($s4 Eh '1'), VAI PARA menu
